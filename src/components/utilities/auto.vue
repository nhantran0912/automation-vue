<template>
  <div class="card border-info" :class="class">
    <h6 class="card-header text-light bg-primary">{{ title }}</h6>

    <div class="card-body" v-if="reloading">
      <p class="card-text" :class="{ 'text-danger' : reloadingError }">{{ reloadingMessage }}</p>
    </div>

    <div class="card-body text-success" v-else-if="!auto.waitingTime">
      <div class="input-group mb-3">
        <input type="text" class="form-control" v-model="auto.id" placeholder="Nhập ID cần hack">
      </div>
      <div class="input-group mb-3">
        <img :src="auto.captchaSrc" class="rounded" alt="captcha">
      </div>
      <div class="input-group mb-3">
        <input type="text" class="form-control" v-model="auto.captcha" placeholder="Nhập mã captcha">
      </div>
